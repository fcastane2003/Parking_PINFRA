...
from app.auth.auth_db import crear_usuario, obtener_usuario_por_username, init_auth_db, listar_usuarios, obtener_usuario_por_id, actualizar_rol_usuario, desactivar_usuario
...
def register(payload: UsuarioCreate, db: Session = Depends(get_db), current_user: dict = Depends(require_roles(["admin"]))):
...
@router.get("/usuarios")
def listar(db: Session = Depends(get_db), current_user: dict = Depends(require_roles(["admin"]))):
...
def actualizar_usuario(user_id: int, payload: UsuarioUpdate, db: Session = Depends(get_db), current_user: dict = Depends(require_roles(["admin"]))):
    if payload.rol:
